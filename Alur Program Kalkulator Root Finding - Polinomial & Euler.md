...
- Jika user memilih `1`:
  ...
  2. Bangun `cmdBuf` = `"simulation.exe"` (path **relatif** — `simulation.exe` harus berada di direktori yang sama dengan `finpro.exe`)
  3. `CreateProcess(NULL, cmdBuf, ..., NULL, NULL, &si, &pi)` — working directory **tidak diset** (`NULL`), menggunakan working directory proses saat ini
...
## 12. JEMBATAN DATA: `writeSimData` & `sim_data.txt`
...
**Path relatif:** `"sim_data.txt"` — ditulis di working directory saat `finpro.exe` dijalankan. `simulation.exe` juga membacanya secara relatif dari working directory yang sama. Jika file gagal dibuat, peringatan dicetak dan simulasi tidak dijalankan.
...
```
- Working directory **tidak diset** (parameter ke-8 = `NULL`) — `simulation.exe` menggunakan working directory proses `finpro.exe` saat dijalankan. Oleh karena itu, `simulation.exe`, `sim_data.txt`, dan folder `resources/` (font) harus berada di direktori yang sama
...
## 13. SIMULASI GRAFIS (`simulation.c` + Raylib)
...
`simulation.c` memerlukan **Raylib** yang tersedia di folder `raylib/src/` (header `raylib.h`, `raymath.h`, dan static library `libraylib.a`). Struktur repositori:
...
Langkah kompilasi `simulation.exe` dari direktori `srcproject/`:
```powershell
gcc simulation.c -o simulation.exe -I../raylib/src -L../raylib/src -lraylib -lopengl32 -lgdi32 -lwinmm
```

**Struktur direktori saat runtime** (semua file harus berada di direktori yang sama saat `finpro.exe` dijalankan):
```
[direktori kerja]/
├── finpro.exe          ← program utama
├── simulation.exe      ← program simulasi grafis (harus sudah di-compile)
├── sim_data.txt        ← ditulis oleh finpro.exe saat runtime, dibaca simulation.exe
└── resources/
    └── minecraft/
        └── Minecraft.ttf
```

> **Urutan penting:** `simulation.exe` harus sudah ada sebelum `finpro.exe` dijalankan dan user memilih tampilkan grafik. Jika belum di-compile, `CreateProcess` akan gagal tanpa pesan error eksplisit ke user — pastikan compile terlebih dahulu. `sim_data.txt` dan `resources/` juga harus dapat diakses dari working directory yang sama.
...
### finpro.c
Setiap `MethodResult` menggunakan `malloc` untuk `rows[]`. Pada versi kode terkini, memori untuk `results[n].rows` **tidak dibebaskan secara eksplisit** di `main` setelah tabel dicetak. Memori akan dibebaskan otomatis oleh sistem operasi saat proses `finpro.exe` berakhir atau loop `do-while` selesai.

> Catatan: Karena program berjalan dalam `do-while` loop dan `cfg = (FunctionConfig){0}` serta `memset(results, 0, sizeof(results))` dilakukan di awal setiap iterasi, pointer lama dari `results[n].rows` akan tertimpa — ini menyebabkan **memory leak** per sesi kalkulasi jika user memilih `Lanjut`. Untuk program pendek bersesi terbatas ini, dampaknya minimal, namun secara prinsip idealnya dipanggil:
> ```c
> for (int i = 0; i < 4; i++) if (results[i].rows) free(results[i].rows);
> ```
> sebelum reset di awal loop berikutnya.
...
| Path relatif `simulation.exe` | `inputShowGraph` | Windows API `CreateProcess` dengan path relatif — `simulation.exe` harus ada di working directory yang sama dengan `finpro.exe` |
...
*Dokumen ini dibuat berdasarkan kode sumber `finpro.c`, `simulation.c`, dan file-file pendukungnya — Program Kalkulator Root Finding Polinomial & Euler, Final Project Praktikum Pemrograman C, Kelompok 5. Terakhir diperbarui: Mei 2026 (refactor multi-file, path relatif, struktur direktori GitHub).*